...
//Program należy samodzielnie podzielić na klasy (w rozsądny sposób). Napisz treść procesów P i K. W rozwiązaniu wykorzystaj blokady.

public class Main {
    public static void main(String[] args) {
        final int ROZMIAR_BUFORA = 10;
        Bufor buffer = new Bufor(ROZMIAR_BUFORA);
        Producent[] producers = new Producent[8];
        Konsument consumer = new Konsument(buffer);

        for (int i = 0; i < producers.length; i++) {
            producers[i] = new Producent(i + 1, buffer);
            producers[i].start();
        }
